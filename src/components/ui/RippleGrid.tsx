...
export function RippleGrid({
  ...
  useEffect(() => {
    ...
    const uniforms = {
      ...
      mouseInfluence: { value: 0 as number },
  ...
  useEffect(() => {
    ...
    if (!u) return;
    (u.enableRainbow as { value: boolean }).value = enableRainbow;
    (u.gridColor as { value: [number, number, number] }).value = hexToRgb(gridColor);
    (u.rippleIntensity as { value: number }).value = rippleIntensity;
    (u.gridSize as { value: number }).value = gridSize;
    (u.gridThickness as { value: number }).value = gridThickness;
    (u.fadeDistance as { value: number }).value = fadeDistance;
    (u.vignetteStrength as { value: number }).value = vignetteStrength;
    (u.glowIntensity as { value: number }).value = glowIntensity;
    (u.opacity as { value: number }).value = opacity;
    (u.gridRotation as { value: number }).value = gridRotation;
    (u.mouseInteraction as { value: boolean }).value = mouseInteraction;
    (u.mouseInteractionRadius as { value: number }).value = mouseInteractionRadius;
  }, [
    enableRainbow,
    gridColor,
    rippleIntensity,
